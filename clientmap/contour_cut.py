...
from count_price import count_price_of_contour
...
class FSMClientContour(StatesGroup):
    size = State()
    amount = State()
    choice = State()
    files = State()
...
async def get_size_contour(message: types.Message, state=FSMContext):
    sticker = data_from_bot.get_content_for_client('sticker_contour')
    await FSMClientContour.size.set()
    await bot.send_sticker(message.chat.id, sticker=sticker[0])
    await bot.send_message(message.chat.id, 'Напишите нужный размер в МИЛЛИМЕТРАХ через пробел.\nПример: 50 70',
    ...
    try:
        ...
        if x.isdigit() and y.isdigit():
            async with state.proxy() as data:
                data['Услуга'] = 'Контурная резка'
                data['Размер'] = f'{x}x{y}'
            await FSMClientContour.amount.set()
            await bot.send_message(message.chat.id, 'Сколько штук вам нужно?')
        else:
...
async def get_amount_contour(message: types.Message, state=FSMContext):
    if message.text.isdigit():
        async with state.proxy() as data:
            data['Стоимость'] = count_price_of_contour(int(data['Размер'].split('x')[0]), int(data['Размер'].split('x')[1]), int(message.text))
        ...
        await FSMClientContour.choice.set()
...
async def get_design_layout(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        if message.text == 'Заказать макет у дизайнера 👩🏻‍🎨':
            data['Дизайнер'] = ''
            await FSMClientContour.design_application.set()
            ...
            try:
                ...
                pass
            await FSMClientContour.files.set()
                ...
                if message.document:
                    ...
                    if message.text == 'Завершить загрузку 📶':
                        ...
                        if len(file_path_os) > 0:
                            ...
                            await FSMClientContour.application.set()
...
def contour_register_handlers(dp: Dispatcher):
    dp.register_message_handler(main_decorator(get_size_contour), state=None)
    dp.register_message_handler(main_decorator(get_choice_design), state=FSMClientContour.size)
    dp.register_message_handler(main_decorator(get_amount_contour), state=FSMClientContour.amount)
    dp.register_message_handler(main_decorator(get_design_layout), state=FSMClientContour.choice)
    dp.register_message_handler(get_layout_files, content_types=['document', 'text'], state=FSMClientContour.files)
    dp.register_message_handler(main_decorator(design_application),
                                state=FSMClientContour.design_application)
    dp.register_message_handler(main_decorator(application), state=FSMClientContour.application)
