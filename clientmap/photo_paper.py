...
from count_price import count_price_of_paper
...
class FSMClientPaper(StatesGroup):
    size = State()
    choice = State()
    files = State()
    application = State()
...
async def get_size_paper(message: types.Message, state=FSMContext):
    await FSMClientPaper.size.set()
    sticker = data_from_bot.get_content_for_client('sticker_paper')
    ...
    try:
        ...
        if x.isdigit() and y.isdigit():
            async with state.proxy() as data:
                data['Услуга'] = 'Фотобумага'
                data['Размер'] = f'{x}x{y}'
                data['Стоимость'] = count_price_of_paper(int(x), int(y))
            ...
            await FSMClientPaper.choice.set()
            await bot.send_sticker(message.chat.id, sticker=sticker[0])
...
async def get_design_layout(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        if message.text == 'Заказать макет у дизайнера 👩🏻‍🎨':
            data['Дизайнер'] = ''
            data['Тираж'] = '-'
            await FSMClientPaper.design_application.set()
            ...
            try:
                ...
                pass
            await FSMClientPaper.files.set()
                ...
                if message.document:
                    ...
                    if message.text == 'Завершить загрузку 📶':
                        ...
                        if len(file_path_os) > 0:
                            ...
                            await FSMClientPaper.application.set()
...
def paper_register_handlers(dp: Dispatcher):
    dp.register_message_handler(main_decorator(get_size_paper), state=None)
    dp.register_message_handler(main_decorator(get_choice_design), state=FSMClientPaper.size)
    dp.register_message_handler(main_decorator(get_design_layout), state=FSMClientPaper.choice)
    dp.register_message_handler(get_layout_files, content_types=['document', 'text'], state=FSMClientPaper.files)
    dp.register_message_handler(main_decorator(design_application),
                                state=FSMClientPaper.design_application)
    dp.register_message_handler(main_decorator(application), state=FSMClientPaper.application)
