...
from handlers.other import main_decorator, list_with_amount_for_poly, list_with_sizes_for_booklets, \
...
class FSMClientBooklets(StatesGroup):
    amount = State()
    size = State()
    type_paper = State()
    choice = State()
...
async def get_amount_booklets(message: types.Message, state=FSMContext):
    ...
    await FSMClientBooklets.amount.set()
...
async def get_size_booklets(message: types.Message, state=FSMContext):
    if message.text.split()[0] in list_with_amount_for_poly:
        ...
        mark_up.add(size_bottom[4])
        await FSMClientBooklets.size.set()
        await bot.send_message(message.chat.id, 'Выберите размер', reply_markup=mark_up)
    else:
        await message.reply('Выберите тираж кнопкой')


async def get_paper_booklets(message: types.Message, state=FSMContext):
    if message.text in list_with_sizes_for_booklets:
        ...
        await FSMClientBooklets.type_paper.set()
...
async def get_choice_design(message: types.Message, state=FSMContext):
    if message.text.split()[2] in list_with_papers_for_poly:
        ...
        await FSMClientBooklets.choice.set()
        await bot.send_sticker(message.chat.id, sticker=sticker[0])
...
async def get_design_layout(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        if message.text == 'Заказать макет у дизайнера 👩🏻‍🎨':
            data['Дизайнер'] = ''
            await FSMClientBooklets.design_application.set()
            ...
            try:
                ...
                pass
            await FSMClientBooklets.files.set()
                ...
                if message.document:
                    ...
                    if message.text == 'Завершить загрузку 📶':
                        ...
                        if len(file_path_os) > 0:
                            ...
                            await FSMClientBooklets.application.set()
...
def booklets_register_handlers(dp: Dispatcher):
    dp.register_message_handler(main_decorator(get_amount_booklets), state=None)
    dp.register_message_handler(main_decorator(get_size_booklets), state=FSMClientBooklets.amount)
    dp.register_message_handler(main_decorator(get_paper_booklets), state=FSMClientBooklets.size)
    dp.register_message_handler(main_decorator(get_choice_design), state=FSMClientBooklets.type_paper)
    dp.register_message_handler(main_decorator(get_design_layout), state=FSMClientBooklets.choice)
    dp.register_message_handler(get_layout_files, content_types=['document', 'text'],
                                state=FSMClientBooklets.files)
    dp.register_message_handler(main_decorator(design_application),
                                state=FSMClientBooklets.design_application)
    dp.register_message_handler(main_decorator(application), state=FSMClientBooklets.application)
