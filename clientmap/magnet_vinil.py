...
from count_price import count_price_of_magnet
...
class FSMClientMagnet(StatesGroup):
    size = State()
    choice = State()
    files = State()
    application = State()
...
async def get_size_magnet(message: types.Message, state=FSMContext):
    await FSMClientMagnet.size.set()
    sticker = data_from_bot.get_content_for_client('sticker_magnet')
    ...
    try:
        ...
        if x.isdigit() and y.isdigit():
            async with state.proxy() as data:
                data['Услуга'] = 'Магнитный винил'
                data['Размер'] = f'{x}x{y}'
                data['Стоимость'] = count_price_of_magnet(int(x), int(y))
            ...
            await FSMClientMagnet.choice.set()
            await bot.send_sticker(message.chat.id, sticker=sticker[0])
...
async def get_design_layout(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        if message.text == 'Заказать макет у дизайнера 👩🏻‍🎨':
            data['Дизайнер'] = ''
            data['Тираж'] = '-'
            await FSMClientMagnet.design_application.set()
            ...
            try:
                ...
                pass
            await FSMClientMagnet.files.set()
                ...
                if message.document:
                    ...
                    if message.text == 'Завершить загрузку 📶':
                        ...
                        if len(file_path_os) > 0:
                            ...
                            await FSMClientMagnet.application.set()
...
def magnet_register_handlers(dp: Dispatcher):
    dp.register_message_handler(main_decorator(get_size_magnet), state=None)
    dp.register_message_handler(main_decorator(get_choice_design), state=FSMClientMagnet.size)
    dp.register_message_handler(main_decorator(get_design_layout), state=FSMClientMagnet.choice)
    dp.register_message_handler(get_layout_files, content_types=['document', 'text'], state=FSMClientMagnet.files)
    dp.register_message_handler(main_decorator(design_application),
                                state=FSMClientMagnet.design_application)
    dp.register_message_handler(main_decorator(application), state=FSMClientMagnet.application)
