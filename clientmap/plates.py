...
from count_price import count_price_of_plates
...
class FSMClientPlates(StatesGroup):
    size = State()
    choice = State()
    files = State()
    application = State()
...
async def get_size_plates(message: types.Message, state=FSMContext):
    sticker = data_from_bot.get_content_for_client('sticker_plates')
    await FSMClientPlates.size.set()
    ...
    try:
        ...
        if x.isdigit() and y.isdigit():
            async with state.proxy() as data:
                data['Услуга'] = 'Таблички'
                data['Размер'] = f'{x}x{y}'
                data['Стоимость'] = count_price_of_plates(int(x), int(y))
            ...
            await FSMClientPlates.choice.set()
...
async def get_design_layout(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        if message.text == 'Заказать макет у дизайнера 👩🏻‍🎨':
            data['Дизайнер'] = ''
            await FSMClientPlates.design_application.set()
            ...
            try:
                ...
                pass
            await FSMClientPlates.files.set()
                ...
                if message.document:
                    ...
                    if message.text == 'Завершить загрузку 📶':
                        ...
                        if len(file_path_os) > 0:
                            ...
                            await FSMClientPlates.application.set()
...
def plates_register_handlers(dp: Dispatcher):
    dp.register_message_handler(main_decorator(get_size_plates), state=None)
    dp.register_message_handler(main_decorator(get_choice_design), state=FSMClientPlates.size)
    dp.register_message_handler(main_decorator(get_design_layout), state=FSMClientPlates.choice)
    dp.register_message_handler(get_layout_files, content_types=['document', 'text'], state=FSMClientPlates.files)
    dp.register_message_handler(main_decorator(design_application),
                                state=FSMClientPlates.design_application)
    dp.register_message_handler(main_decorator(application), state=FSMClientPlates.application)
