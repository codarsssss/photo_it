...
from count_price import count_price_of_city
...
class FSMClientCity(StatesGroup):
    size = State()
    choice = State()
    files = State()
    application = State()
...
async def get_size_city(message: types.Message, state=FSMContext):
    await FSMClientCity.size.set()
    sticker = data_from_bot.get_content_for_client('sticker_city')
    ...
    try:
        ...
        if x.isdigit() and y.isdigit():
            async with state.proxy() as data:
                data['Услуга'] = 'City Lite'
                data['Размер'] = f'{x}x{y}'
                data['Стоимость'] = count_price_of_city(int(x), int(y))
            ...
            await FSMClientCity.choice.set()
            await bot.send_sticker(message.chat.id, sticker=sticker[0])
...
async def get_design_layout(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        if message.text == 'Заказать макет у дизайнера 👩🏻‍🎨':
            data['Дизайнер'] = ''
            data['Тираж'] = '-'
            await FSMClientCity.design_application.set()
                ...
                if message.document:
                    ...
                    if message.text == 'Завершить загрузку 📶':
                        ...
                        if len(file_path_os) > 0:
                            ...
                            await FSMClientCity.application.set()
...
def city_register_handlers(dp: Dispatcher):
    dp.register_message_handler(main_decorator(get_size_city), state=None)
    dp.register_message_handler(main_decorator(get_choice_design), state=FSMClientCity.size)
    dp.register_message_handler(main_decorator(get_design_layout), state=FSMClientCity.choice)
    dp.register_message_handler(get_layout_files, content_types=['document', 'text'], state=FSMClientCity.files)
    dp.register_message_handler(main_decorator(design_application),
                                state=FSMClientCity.design_application)
    dp.register_message_handler(main_decorator(application), state=FSMClientCity.application)
